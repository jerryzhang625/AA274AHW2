...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        # Detect Lower & Upper Bound Violations
        if x[0] < self.statespace_lo[0] or x[1] < self.statespace_lo[1]:
            return False
        elif x[0] > self.statespace_hi[0] or x[1] > self.statespace_hi[1]:
            return False

        # Detect Obstacle Violations - pulling fuction from DetOccupancyGrid2D class
    ...
    def distance(self, x1, x2):
        ...
        """
        ########## Code starts here ##########
        dist = np.linalg.norm(np.array(x1)-np.array(x2)) # Inspiration from geeksforgeeks.org
        return dist
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        # Add / Subtract unit distance from x
        neighbor1 = self.snap_to_grid(([x[0] + self.resolution, x[1] + self.resolution]))
        if self.is_free(neighbor1):
            neighbors.append(neighbor1)
        neighbor2 = self.snap_to_grid(([x[0] + self.resolution, x[1] - self.resolution]))
        if self.is_free(neighbor2):
            neighbors.append(neighbor2)
        neighbor3 = self.snap_to_grid(([x[0] - self.resolution, x[1] + self.resolution]))
        if self.is_free(neighbor3):
            neighbors.append(neighbor3)
        neighbor4 = self.snap_to_grid(([x[0] - self.resolution, x[1] - self.resolution]))
        if self.is_free(neighbor4):
            neighbors.append(neighbor4)
        neighbor5 = self.snap_to_grid(([x[0], x[1] + self.resolution]))
        if self.is_free(neighbor5):
            neighbors.append(neighbor5)
        neighbor6 = self.snap_to_grid(([x[0], x[1] - self.resolution]))
        if self.is_free(neighbor6):
            neighbors.append(neighbor6)
        neighbor7 = self.snap_to_grid(([x[0] + self.resolution, x[1]]))
        if self.is_free(neighbor7):
            neighbors.append(neighbor7)
        neighbor8 = self.snap_to_grid(([x[0] - self.resolution, x[1]]))
        if self.is_free(neighbor8):
    ...
    def solve(self):
        ...
        """
        ########## Code starts here ##########
        # Implement A star Motion Plannin Algorithm
        # Initiallization already completed in previous parts
        while len(self.open_set)>0:
            x_current = self.find_best_est_cost_through() # Find lowest cost option in open set
            ...
            # Remove and add x_current from open and closed set
            self.open_set.remove(x_current)
            self.closed_set.add(x_current)
            
            # Fetch Neighbor to construct feasible route
            neighbor_list = self.get_neighbors(x_current)
            for x_neigh in neighbor_list:
            ...
            # Populate lists with stats at x_neigh
        ...
        return False # If path search fails
